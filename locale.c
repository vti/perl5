...
#  endif /* USE_LOCALE_NUMERIC */
                ...
                /* Look through the environment for any variables of the form
                 * qr/ ^ LC_ [A-Z]+ = /x, except LC_ALL which was already
                 * handled above.  These are assumed to be locale settings.
                 * Output them and their values. */
                for (e = environ; *e; e++) {
                    const STRLEN prefix_len = sizeof("LC_") - 1;
                    STRLEN uppers_len;

                    if (      strSTARTS_WITHs(*e, "LC_")
                         && ! strSTARTS_WITHs(*e, "LC_ALL=")
