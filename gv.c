...
Perl_gv_add_by_type(pTHX_ GV *gv, svtype type)
{
	...
	*where = newSV_type(type);
	    if (type == SVt_PVAV
	     && strEQs(GvNAME(gv), GvNAMELEN(gv), "ISA"))
      ...
      have_gv:
		...
		cand_cv = NULL;
	      ...
	      && len > 1 /* shortest is uc */
              && strEQs(hvname, HvNAMELEN_get(stash), "CORE")
    ...
    while (items--) {
        ...
        if (!gvp) {
            if (len > 1 && HvNAMELEN_get(cstash) == 4) {
                ...
                if (strSTARTS_WITHs(hvname, "CORE")
                 && (candidate =
...
Perl_gv_fetchmethod_pvn_flags(pTHX_ HV *stash, const char *name, const STRLEN len, U32 flags)
{
    ...
    if (last_separator) {
	    ...
	    DEBUG_o( Perl_deb(aTHX_ "Treating %s as %s::%s\n",
		 ...
		 strSTARTS_WITHs(last_separator - 7, "::SUPER")) {
                      ...
                      const svtype sv_type)
{
	...
	if (len > 1 /* shortest is uc */ && HvNAMELEN_get(stash) == 4) {
	  ...
	  if (strSTARTS_WITHs(stashname, "CORE"))
...

  {
	...
	gv = Perl_gv_fetchmeth_pvn(aTHX_ stash, cooky, l, -1, 0);
        ...
        if (gv && (cv = GvCV(gv)) && CvHASGV(cv)) {
            ...
            if (strEQs(HEK_KEY(gvhek), HEK_LEN(gvhek), "nil")
             && stashek
             && strEQs(HEK_KEY(stashek), HEK_LEN(stashek), "overload")) {
